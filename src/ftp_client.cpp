...
CFTPClient::CFTPClient() : m_filename(""), m_filesize(-1), m_is_rest(false), m_file_offset(0)
{
...
bool CFTPClient::set_port_mode()
{
    ...
    {
        close(sockfd);
        std::cout << "fail to set port mode, bind error";
        return false;
    }
    ...
    {
        close(sockfd);
        std::cout << "fail to set port mode, listen error";
        return false;
    }
...
//    std::cout << "127.0.0.1" << ":" << p1 * 256 + p2 << std::endl;
...
bool CFTPClient::download(std::string& argument)
{
    int n = argument.size();
    if (argument[n - 1] == '/') argument.pop_back();
    std::string::size_type idx = argument.find_first_of(' ', 0);
    std::string filename = argument.substr(0, idx);
    std::string path = argument.substr(idx + 1);
...
//    std::cout << control << std::endl;
    ...
    std::cout << m_filename << " " << m_filesize << std::endl;
    pthread_t tid;
...
{
    std::cout << "in process download" << std::endl;
    ...
    long long int recv_size = ftp_client->m_filesize;
    std::string message;
    while (true)
    {
